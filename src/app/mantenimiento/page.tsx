...
const GestionMantenimiento: React.FC = () => {
  ...
  const obtenerNombreEncargado = (idEncargado: number): string => {
    const encargado = personas.find((persona) => persona.id_persona === idEncargado);
    return encargado ? encargado.nombre: "No asignado";
  }
  const obtenerNombreEspacio = (idEspacio: number): string => {
    const espacio = espacios.find((espacio) => espacio.id_espacio === idEspacio);
  ...
  // Filtrar mantenimientos según el rol
  /*
  const mantenimientosVisibles = mantenimientos.filter(mantenimiento => {
    if (rolSimulado === "admin") return true;
    // Aquí puedes agregar más lógica de filtrado según otros roles
    return true;
              ...
              <tbody>
                {mantenimientos.map((mantenimiento) => (
                  <tr key={mantenimiento.id_mantenimiento}>
                    ...
                    <td className="border border-gray-300 p-2">
                      <span className={`px-2 py-1 rounded ${mantenimiento.estado === 'Pendiente' ? 'bg-yellow-200' :
                          mantenimiento.estado === 'En Progreso' ? 'bg-blue-200' :
                            'bg-green-200'
                        }`}>
